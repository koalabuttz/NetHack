# Campaign bench plan (Revision 4 — DRAFT, pending plan review)
...
- **Termination-safety admission contract (required before any apply).** Because entered cells can be gamed by reckless play, predeclare a **deterministic-only** termination-safety admission evaluated on every comparison and required before any `apply-approved`. Every episode is first classified by the **terminal-classification table**, which is the only authority for adverse-termination rates:
  ...
  | 3 | `stop_reason` | bench interruption (operator stop-file / SIGINT) | excluded-from-comparison |
  | 4 | `stop_reason` | deadline / protocol / transport / spawn / recorder failure | operational/integrity failure |
  | 5 | `stop_reason` | `closed-unanswered` | operational/integrity failure |
  | 6 | `stop_reason` | `closed` (generic — the value a real death almost always carries) | fall through to `outcome` (rows 7–10) |
  | 7 | `outcome` (generic `closed` only) | `death` | adverse-early (observed, conservative) |
  | 8 | `outcome` (generic `closed` only) | `starvation` | adverse-early (observed, conservative) |
  | 9 | `outcome` (generic `closed` only) | `ascension` | explicit non-adverse category `ascension` |
  | 10 | `outcome` (generic `closed` only) | any other / unknown | adverse/unknown (counts against admission) |

  The classifier is defined over these **exact artifact values**, not over labels the controller never emits. `controller.py:1058–1070,1089–1108` shows the controller emits `closed`, `tick-cap-graceful-quit`, `policy-exhausted`, deadline/protocol/transport/spawn/recorder failures, and `closed-unanswered` — it **never** emits `death` or `starvation` as a `stop_reason`. Death and starvation exist only as `recording.infer_outcome()` values (`recording.py:329–341`), and a real death is typically `stop_reason="closed", outcome="death"`. So rows 1–6 key on the trusted `stop_reason`, and only a generic `closed` consults the visible `outcome` (rows 7–10). Visible-text `outcome` is **observational, not proof** — a heuristic inference (`recording.py:329–341`) — but it **is** usable for the **conservative safety rate**: `death`/`starvation` outcomes count as adverse observed outcomes, and any unrecognized `outcome` is adverse/unknown. `ascension` gets its own explicit non-adverse category rather than being folded into unknown or horizon-completion.
  - **Noninferiority limits** on the adverse/unknown rates — `policy-exhausted`, generic-`closed` `death`/`starvation` outcomes, and the unknown class — at operator-approved absolute noninferiority margins. Bench interruption (excluded-from-comparison), operational/integrity failures (rows 4–5), and explicit `ascension` are **not** safety-rate events.
...
- **Second/forced path — ownership-isolated process-tree walk (no controller change, no systemd dependency).** If the bounded wait elapses or a descendant ignores TERM, reap **only the owned tree**:
  ...
  - **Failure is fatal, not cosmetic.** A **permission or identity-validation failure** sets **teardown-failure / partial** status and **FAILS AC7**, even when cleanup continues best-effort. (This replaces the earlier "logged, never fatal" wording.)
...
AC11. The termination-safety admission contract (§2) is predeclared, deterministic and blocks any apply: episodes are classified by the **terminal-classification table over exact artifact values** — authoritative `stop_reason` values (`policy-exhausted` → adverse-early; `tick-cap-graceful-quit` → horizon-completion; bench interruption → excluded-from-comparison; deadline/protocol/transport/spawn/recorder failures and `closed-unanswered` → operational/integrity failure) and, only for a generic `stop_reason="closed"`, the visible-text `outcome` (`death`/`starvation` → adverse-early observed; `ascension` → explicit category; else adverse/unknown). A candidate that raises the adverse/unknown rate beyond the approved noninferiority margin fails; an arm below any predeclared active exposure minimum yields **inconclusive** (not not-comparable); neither passes on coverage gain.
...
- `test_unknown_outcome_handled_by_precedence_table`: production-shaped `(stop_reason, outcome)` pairs — `("policy-exhausted", *)` → adverse-early, `("tick-cap-graceful-quit", *)` → horizon-completion, `("closed", "death")` → adverse-early, `("closed", "ascension")` → explicit `ascension`, `("closed", "<unrecognized>")` → adverse/unknown, and a bench-interruption `stop_reason` → excluded-from-comparison.
