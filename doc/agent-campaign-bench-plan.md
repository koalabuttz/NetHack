# Campaign bench plan (Revision 3 — DRAFT, pending plan review)
...
**Provenance domains (split).** Two domains are tracked separately because they have different comparability consequences:

- **Deterministic domain** — controller/provider gameplay code, the bench runner/metrics/comparison code, the worker/launcher binaries, the data/sysconf trees, and the **metric/exclusion policy**. A behavior-affecting difference here (a different commit id, or a different hash of any imported bench/agent Python module, worker/launcher binary, data/sysconf manifest, or metric-policy version) is **hard not-comparable** — even when the commit id is unchanged (a dirty tree that alters a relevant module is not-comparable).
...
The manifest records all three domains so a comparison can state precisely which difference drove its verdict, and so judge provenance can never masquerade as a deterministic blocker.
...
- **Termination-safety admission contract (required before any apply).** Because entered cells can be gamed by reckless play, predeclare a **deterministic-only** termination-safety admission evaluated on every comparison and required before any `apply-approved`. Every episode is first classified by the **terminal-classification table**, which is the only authority for adverse-termination rates:
  ...
  | Precedence | Authority | Category | Class |
  |---|---|---|---|
  | 1 | trusted `stop_reason` | `death` | adverse (early) |
  | 2 | trusted `stop_reason` | starvation | adverse (early) |
  | 3 | trusted `stop_reason` | policy-exhausted-before-horizon | adverse (early) |
  | 4 | trusted `stop_reason` | tick-cap-horizon-completion | horizon completion (non-adverse) |
  | 5 | trusted `stop_reason` | operator-interruption | excluded (not a game outcome) |
  | 6 | trusted `stop_reason` | transport-failure | excluded (operational; integrity gate) |
  | 7 | visible-text `outcome` | any value | **not proof** (`recording.py:329–341`); advisory only |
  | 8 | none | unknown / unclassified | **adverse/unknown** (counts against admission) |

  The trusted `stop_reason` category is consulted **first** and is authoritative; a visible-text `outcome` never overrides it and is **not** treated as proof (`recording.py:329–341`); anything that does not resolve to a trusted `stop_reason` category is **unknown** and treated as adverse/unknown (row 8), never benign.
  - **Noninferiority limits** on the adverse/unknown rates — `death`, starvation, `policy-exhausted-before-horizon`, and the unknown class — at operator-approved absolute noninferiority margins.
  - **Minimum common exposure (numeric).** The spec predeclares a minimum at-risk exposure per arm — `min_completed_episodes_per_arm` and/or `min_aggregate_at_risk_ticks_per_arm`. **All** early adverse terminals are counted in the rate denominators (never discard the adverse event). If either arm's aggregate at-risk exposure is below the predeclared minimum, the comparison is **inconclusive** (insufficient aggregate exposure) — **not** not-comparable.
  - **Unknown outcomes:** an unknown `game_outcome` or an unclassified terminal counts as **adverse/unknown evidence** (inconclusive at best), never a pass or a benign terminal; a visible-text `outcome` alone never rescues a missing trusted `stop_reason` classification.
...
- **Second/forced path — bench-owned process-tree walk (no controller change, no systemd dependency).** If the bounded wait elapses or a descendant ignores TERM, the bench reaps the whole tree itself: starting from the bench child's PID it walks `/proc` by **PPID recursion** to enumerate **all** descendants — the controller child, its launcher session leader, and every descendant. The controller's `start_new_session` launcher is a *different session* but IS a `pid`-tree descendant, so PPID recursion still reaches it and its group. It then `SIGKILL`s **each discovered descendant** and **each discovered process group** (`getpgid`). Documented limits: an inherent **race window** (descendants can spawn mid-walk; re-walk until the tree is stable or the bound elapses), **PID reuse** (validate identity/start time before signalling a reused PID), and **permission failures** (logged, never fatal).
- **Test:** `test_forced_abort_reaps_nested_launcher_and_provider_groups` drives bench child → controller launcher session → a **TERM-ignoring descendant** and asserts **every PID is gone** after the forced path (no survivors in the tree), exercising the PPID-recursion walk.
...
3. **Runner and operational controls:** dry-run integration, one-episode campaign wrapper, atomic checkpoints, caps/reservations and stop behavior (including the §7 forced-abort containment mechanism — child SIGINT/SIGTERM → controller `_reap`, plus the bench-owned `/proc` PPID-recursion process-tree walk for the forced path). Prove fake-child process cleanup and actual-tier detection. Gate strict live-Jev support as described rather than weaken preflight.
...
AC11. The termination-safety admission contract (§2) is predeclared, deterministic and blocks any apply: episodes are classified by the terminal-classification table (trusted `stop_reason` first, then visible-text `outcome` — not proof — then unknown). A candidate that raises the adverse/unknown rate (`death` / starvation / `policy-exhausted-before-horizon` / unknown) beyond the approved noninferiority margin fails; an arm whose aggregate at-risk exposure is below the predeclared minimum yields **inconclusive** (not not-comparable); neither passes on coverage gain.
...
- `test_policy_exhausted_regression_blocks_admission`
...
Each mutation killed by its named test: scorecard silently coerces null→zero (AC1); dry-run performs a provider call (AC2); fallback-only live run passes tier validation (AC3); hard-gate failure offset by target improvement (AC4); postmortem reserve not forced to zero (AC6); stop admits the next episode (AC7); tuner peeks until success / exceeds candidate budget (AC8); apply without approval/hash/range (AC9); confidence factor auto-tuned without specific approval (AC9/D3); interrupted run promoted to baseline (AC7); termination-safety admission ignored so a coverage-gaining reckless candidate passes (AC11); early stop on a favorable interim look or a post-result margin/sample change (AC8); a coverage-gaining candidate accepted despite a higher death / policy-exhausted / unknown rate (AC11); a judge rubric-only change invalidating the deterministic comparison or blocking apply (AC6).
