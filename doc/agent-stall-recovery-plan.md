...
| held/acquiring | any destination-acquire/continue **movement** | recognized blocking movement confirmation (`into that … cloud?`), same instance, unchanged confirmed hero | **attempt 1 counted at prompt arrival** (never again for the answer result) | destination route/stall charge remains exactly once; a bound `n` learns the origin edge (§D) and re-applies neither the destination effect nor an applied token |
| held/acquiring | any destination movement | recognized blocking confirmation, but hero moved / instance changed / unmatched origin | not advanced | not charged |

Rules:

1. Only matched gameplay attempts advance the stationary 3/6/10 stage; prompt, inventory, and unmatched observations do not. The single exception is a matched movement attempt whose immediate response is a recognized blocking movement confirmation — counted once at arrival, identity-bound, and never again for its answer.
...
7. A matched blocking movement confirmation is counted **exactly once, at prompt arrival**, and only when the frozen origin edge, the instance and the unchanged confirmed hero all hold; the answer's own result never counts again, and an unrelated `yn`, an interaction-direction prompt or an unmatched frame never counts (prompt-edge plan §A/§B).
8. A confirmed **decline** (`n` sent, the confirmation dismissed, the confirmed hero unchanged at the frozen source in the same instance) records typed `prompt-declined` evidence for the **origin directed edge** — never for the answer action or the semantic destination — in the scoped blocked-edge ledger, keyed by directed edge plus movement action class, one record per edge/class, cleared on instance reset (prompt-edge plan §C/§D). A stale `n`, a failed write, a replacement prompt or a re-presented confirmation writes nothing; the answer never re-applies the destination effect or spends an applied token.

Initialize route progress from the hero baseline and carry the initial route hop count into the total cap (§2). Named tests: `test_prompt_bound_attempts_compose_with_stationary_thresholds_3_6_10` (the split successor to the former broad `test_prompt_observations_do_not_advance_stationary_stage`, which no longer holds for a *matched movement* confirmation while every genuinely unmatched prompt remains excluded — see `test_unrelated_yn_and_interaction_direction_do_not_advance_stationary` and `test_prompt_replay_unknown_hero_and_instance_change_do_not_double_count`), `test_first_no_time_acquisition_is_attempt_one_of_three`, `test_move_to_door_approach_is_not_an_interaction`, `test_matched_movement_vapor_prompt_advances_stationary_once`, `test_prompt_decline_requires_matching_answer_send_and_resolution`.
...
- `prompt_declined_record(instance, src, dst, action_class)`: the prompt-edge plan's typed extension. Adds a `prompt-declined` reason to the same ledger: the current `blocked_edge_signature` components plus the normalized exact prompt text, keyed by the **directed edge plus movement action class** (destination navigation, random movement and bounded recovery share the *normal* class; only the emergency class is distinct). One bounded record per directed edge/action class (overwritten), cleared on instance reset. **Under the recorded exit-(b) disposition** (the gray `#` vapor folds to `T_CORRIDOR`, so no local overlay token is observable) the record suppresses **regardless of any local signature change** — the stored signature and prompt text are retained for diagnostics only — and an instance/scope reset is the only reopening. Signature-based reopening is gated behind the explicit positive-reopening capability, which stays disabled until the mandatory operator-gated cloud trace. The edge never reopens on time, visits, serials, global revisions, unrelated messages or remote occupancy. Named tests: `test_prompt_declined_edge_survives_prompt_absence_and_destination_reacquisition`, `test_exit_b_suppression_survives_local_signature_changes`, `test_positive_reopening_capability_gates_signature_reopening`, `test_prompt_edge_ignores_time_visits_remote_occupancy_and_occlusion`, `test_prompt_ledger_overwrites_per_edge_and_clears_on_instance_reset`.
...
- A blanket counter on all stationary observations would count prompts, inventory and harmless interruptions. Prefer matched gameplay-attempt accounting while retaining existing observation position evidence. The prompt-edge plan's narrow exception (a *matched movement* confirmation counted once at arrival) is identity-bound and never a blanket `yn` count.
