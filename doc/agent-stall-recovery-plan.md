...
- `prompt_declined_record(instance, src, dst, action_class)`: the prompt-edge plan's typed extension. Adds a `prompt-declined` reason to the same ledger: the current `blocked_edge_signature` components plus the normalized exact prompt text, keyed by the **directed edge plus movement action class**. One bounded record per directed edge/action class (overwritten), cleared on instance reset; the blocked-edge view is queried **per action class**, so a normal-navigation decline never suppresses the same edge for the emergency class. Classified terrain alone is insufficient for clouds (a gray `#` vapor folds to `T_CORRIDOR`), so the minimum public edge-local overlay token is retained and the edge stays conservatively suppressed under unchanged evidence until a positive relevant local change or a scope change — never on time, visits, serials, global revisions, unrelated messages or remote occupancy. Named tests: `test_prompt_declined_edge_survives_prompt_absence_and_destination_reacquisition`, `test_prompt_edge_reopens_on_positive_local_cloud_change`, `test_prompt_edge_ignores_time_visits_remote_occupancy_and_occlusion`, `test_prompt_edge_signature_preserves_diagonal_side_evidence`, `test_prompt_ledger_overwrites_per_edge_and_clears_on_instance_reset`.
