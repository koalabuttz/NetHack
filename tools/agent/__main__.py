...
def cmd_auto(a) -> int:
    ...
    failures = 0
    for r in results:
        ok = r.closed and not r.forced_kill and not r.eof \
            and r.protocol_failure is None and r.recording_complete
        if not ok:
            failures += 1
        print("episode %d: stop=%s outcome=%s ticks=%d needs=%d actions=%d "
              "invalids=%d closed=%s recording_complete=%s rc=%s"
              % (r.index, r.stop_reason, r.outcome, r.ticks, r.needs,
                 r.actions, r.invalids, r.closed, r.recording_complete,
                 r.returncode))
